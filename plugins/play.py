...
async def play(_, message: Message):
    ...
    if audio:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("🚨 قناة السورس", url="t.me/music_Desha1"),
                    InlineKeyboardButton(" مبرمج السورس يبني..📚", url="t.me/DeshaXBlacck"),
                ],
        ...
        if (dur / 60) > DURATION_LIMIT:
            await lel.edit(
                f"❌ مقاطع الفيديو أطول من {DURATION_LIMIT} minutes aren't allowed to play!"
            )
            return
        requested_by = message.from_user.first_name
        await generate_cover(requested_by, title, views, duration, thumbnail)
        ...
        if len(message.command) < 2:
            return await lel.edit(
                "🧐 **لم يتم العثور على الاغنية! Try searching with the correct title\nExample » /play In The End\n\nChannel : @DeCodeMusicBot**"
            )
